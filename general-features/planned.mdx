...
A task appears in the Planned page if it has a due date and is not completed or cancelled. Tasks that have only a start date (but no due date) do not appear here.

To add a task to the Planned view, set a due date from the task detail page.
