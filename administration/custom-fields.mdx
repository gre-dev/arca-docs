...
By default, a custom field is visible to all workspace members. You can restrict a field to specific members by setting an **Allowed users** list in the field settings. Members not on the list cannot see the field on tasks — it is completely hidden from their task sidebar and cards. Only members explicitly included in the list can see and edit the field on tasks, including workspace owners unless they are listed.

Workspace owners and admins always see every field in **Settings → Custom Fields** so they can manage the workspace configuration.
